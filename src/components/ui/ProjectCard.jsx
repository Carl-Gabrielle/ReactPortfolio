...
export const ProjectCard = ({ project, onClick, isFirst }) => (
    <div
        className="relative z-20  w-full h-72 bg-white  rounded-tl-3xl rounded-br-3xl hover:scale transition-transform duration-300 group overflow-hidden dark:bg-containerDark dark:border-0"
    >
        ...
        <div className="absolute bottom-4 left-4 z-20">
            <span className="text-primary text-2xl font-bold opacity-80 dark:text-slate-200">{project.count}</span>
        </div>
        {
            isFirst && (
        ...
        }
        {
            !isFirst && (
                <div
                    onClick={onClick}
                    role="button"
                    className="absolute bottom-6 right-6 z-20 flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-white/90 rounded-lg shadow-sm border border-gray-100 hover:shadow-md transition-all duration-200 ease-in-out"
