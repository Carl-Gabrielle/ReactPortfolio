...
export default function Navbar() {
  ...
  return (
    <>
      <header className="w-full h-20 sticky top-0 z-30 bg-white dark:bg-dark bg-opacity-60 backdrop-blur-md shadow-md transition-all duration-300 ease-in-out">
        <nav className="max-w-8xl mx-auto px-6 h-full flex items-center justify-between">
          ...
          >
            <div
              id="nameRole"
              className="flex flex-col items-start text-primary dark:text-slate-100"
            >
            ...
            {navItems.map((item) => (
              <li id="navs" key={item.label} className="flex justify-center">
                <button
                  ...
                  className={`transition-all duration-200 hover:underline font-semibold underline-offset-4 ${
      ...
      {menuOpen && (
        <div
          className={`fixed inset-0 z-30 transition-all duration-200 ease-in-out ${
            fadeIn ? "bg-black/50" : "bg-black/0 backdrop-blur-0"
          }`}
          onClick={closeMenu}
        >
          <div
