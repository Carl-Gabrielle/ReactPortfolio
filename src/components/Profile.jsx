...
const EducationCard = ({ logo, title, date, school, program, isLast }) => (
  <div className="relative group pl-8 education-card">
    <div className="absolute left-0 top-5 h-3 w-3  rounded-full bg-gradient-to-r from-blue-500 to-indigo-600 border-2 border-white shadow-md transform -translate-x-1/2 z-30 transition-all duration-300 group-hover:scale-125"></div>
    ...
    <div className="rounded-xl shadow-sm p-4 flex items-start gap-4 transition-all duration-300 hover:shadow-md hover:-translate-y-0.5 bg-white dark:bg-containerDark">
      ...
      <div className="flex-1">
        ...
        <p className="text-gray-600 text-xs font-medium mt-1 dark:text-slate-200">
          {date}
        </p>
        <p className="text-gray-500 text-xs mt-0.5 dark:text-slate-200">
          {program}
        </p>
        <p className="text-gray-700 text-xs mt-1.5 dark:text-slate-200">
          {school}
        </p>
      </div>
    </div>
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="border border-gray-100 bg-white rounded-3xl p-6 shadow-sm dark:bg-containerDark dark:border-0">
          <SectionTitle icon={<PiGraduationCap size={20} />} text="Education" />
