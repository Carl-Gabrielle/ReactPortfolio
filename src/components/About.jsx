...
const CardStyle = ({ children, className = "" }) => (
  <div
    className={`group p-6 rounded-xl border border-gray-300 transition-all duration-300 shadow-sm hover:shadow-md ${className}`}
  >
    {children}
  </div>
);
...
const ServiceCard = ({ icon, title }) => (
  <div className="service-card  services-card px-3 py-2  rounded-lg border border-gray-400 border-dashed dark:border-slate-200    shadow-sm hover:shadow-md transition-shadow dark:text-slate-200 dark:transition-all duration-300 ease-in-out">
    <div className="flex items-center space-x-3">
      ...
      <h4 className=" font-medium text-gray-800 text-xs sm:text[2em] text-nowrap dark:text-slate-200 transition-all duration-300 ease-in-out">
        {title}
      </h4>
    </div>
  </div>
  ...
  <p className="text-gray-700 text-base mt-4 leading-8 text-justify dark:text-slate-200 transition-all duration-300 ease-in-out">
    {textParts.map((part, index) =>
      part.bold ? (
        <strong
          key={index}
          className="font-semibold text-gray-700 dark:text-slate-200 transition-all duration-300 ease-in-out"
        >
          {part.content}
        </strong>
      ) : (
...
export default function About() {
  ...
  return (
    <section
      ref={sectionRef}
      id="about"
      className="w-full h-full dark:bg-dark py-10  md:py-9 px-6  max-w-full  mx-auto scroll-mt-20 transition-all duration-300 ease-in-out relative"
    >
        ...
        <div className="space-y-4 grid col-span-2 ">
          ...
          <ParagraphBlock
            textParts={[
              {
                content:
                  "I specialize in building modern, responsive web solutions using technologies such as ",
                bold: false,
              },
              ...
              {
                content: "for efficient, scalable styling, as well as ",
                bold: false,
              },
              { content: "GSAP ", bold: true },
              {
                content: "for creating smooth, engaging animations. ",
                bold: false,
              },
            ]}
          />
          ...
          <ParagraphBlock
            textParts={[
              {
                content:
                  "In addition to working on personal projects, I thrive in collaborative environments, leveraging ",
                bold: false,
              },
              ...
              {
                content:
                  "for version control and contributing effectively to team-based development workflows. My focus is on writing clean, maintainable code while continuously expanding my skills to stay aligned with evolving industry standards.",
                bold: false,
              },
            ]}
          />
            ...
            <div className="mb-4">
              ...
              <div className="flex items-center space-x-2 text-gray-500">
                ...
                <span className="text-sm font-medium dark:text-slate-200">
                  I build user-friendly websites with a focus on design,
