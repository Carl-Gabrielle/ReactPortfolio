...
export default function Hero() {
  ...
  const StatisticCard = ({ number, description, isLast }) => (
    <div
      className={`px-8 sm:px-12 lg:px-16 flex flex-col items-center border-gray-400 dark:border-slate-200  transition-all duration-300 ease-in-out  ${!isLast ? "sm:border-r-2 border-dashed" : ""}`}
    >
  ...
  const PrimaryBadge = ({ title }) => {
    return (
      <div
        id="primBadge"
        className="inline-flex items-center gap-2 px-3 py-2 text-sm rounded-full bg-gradient-to-r from-green-500 to-emerald-600 text-white font-medium tracking-wide shadow-md  hover:shadow-lg"
      >
  ...
  return (
    <>
      <section
        id="hero"
        className="w-full h-full   dark:bg-dark  py-10  md:py-9 px-6  max-w-full  mx-auto relative scroll-mt-20 transition-all duration-300 ease-in-out"
      >
        <div className="flex flex-wrap gap-3 justify-center md:justify-start">
          <PrimaryBadge title="Open to Web Development Projects" />
          <SecondaryBadge title="React & Laravel " />
        </div>
        <div className="grid grid-cols md:grid-cols-1 lg:grid-cols-2 gap-8 items-center w-full py-12 ">
          <div className="flex justify-center   lg:order-2 w-full relative">
            ...
            <div
              id="address"
              className="hidden md:flex absolute -bottom-3 left-3 items-center gap-2 px-3 py-2 dark:border-containerDark  dark:bg-containerDark  bg-white/60 backdrop-blur-sm rounded-full border border-gray-200/80 shadow-sm hover:shadow dark:transition-all dark:duration-300 group max-w-fit"
            >
              <HiLocationMarker
              ...
              <div className="flex flex-col leading-tight ">
                <span className="text-sm font-medium text-gray-900 dark:text-slate-200  dark:transition-all dark:duration-300 dark:ease-in-out">
                  Alaminos City, Pangasinan
                </span>
                <span className="text-xs text-gray-500 dark:text-slate-200 dark:transition-all dark:duration-300 dark:ease-in-out">
                  {" "}
                  Philippines • 22 y/o • 1 Year Experience
                </span>
              </div>
            </div>
          </div>
          <div className="space-y-5 order-2 md:order-1 text-center md:text-left w-full ">
            <h1
              ...
              className="  text-7xl  text- tracking-wide font-extrabold mb-2 dark:text-slate-200 transition-all duration-300 ease-in-out"
            >
              Hello,{" "}
            ...
            >
              ...
              />{" "}
              graduate with a strong foundation in software development and
              modern web technologies. A passionate{" "}
              <BlurText
                text="Web Developer,"
                delay={150}
                animateBy="words"
                ...
                className="font-semibold text-nowrap text-primary dark:text-slate-200 dark:transition-all dark:duration-300 dark:ease-in-out"
              />{" "}
              committed to building scalable, user-focused applications using
              modern frameworks and best practices. Continuously learning and
              staying current with industry trends, I am eager to contribute
              technical expertise and problem-solving skills to impactful and
              innovative projects.
            </p>
            ...
            <div className="w-full flex items-center justify-between gap-4">
              ...
              <ul
                id="socialLinks"
                className="flex items-center gap-4 text-gray-700 dark:text-slate-200 dark:transition-all dark:duration-300 dark:ease-in-out"
              >
                <li>
                  <a
